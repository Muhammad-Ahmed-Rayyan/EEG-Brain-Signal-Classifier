...
# --------------------------------------------------
# 3)  Load trained model (Random Forest)
# --------------------------------------------------
rf = joblib.load(
    "rf_model_randomForest_n100_3_users.joblib"
)  # Update filename if needed

# --------------------------------------------------
# 4)  Predict on a new trial
# --------------------------------------------------
sample_raw = load_json_sample("./dataset/sameed/right/sameed_right75.json")
sample_feat = extract_eeg_features(sample_raw)  # shape (48,)

prediction = rf.predict([sample_feat])[0]
